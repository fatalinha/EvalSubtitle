...
import util.degrade_tagged_txt
import util.degrade_txt
...
def main(args):
    ...
    mode = "mixed"
    out_dir_path = os.path.join(out_dir_path, mode)
    if not os.path.exists(out_dir_path):
        os.makedirs(out_dir_path)

    # Init dictionary to store metrics
    eval_metrics = {"alpha": list(),
    ...
    for p_txt in range(10, 100, 10):
        p_add = p_del = p_rep = p_txt / 3
        mixed_txt_file_path = os.path.join(out_dir_path, "amara.mixed_txt.%d.txt" % p_txt)
        n_words = util.degrade_txt.mixed(ref_file_path, mixed_txt_file_path, p_add, p_del, p_rep)

        for p_tags in range(10, 100, 10):
            p_eol_add = p_eol_del = p_eol_rep = p_tags / 3
            p_eob_add = p_eob_del = p_eob_rep = p_tags / 3
            mixed_tags_file_path = os.path.join(out_dir_path, "amara.mixed_txt.%d.mixed_tags.%d.txt" % (p_txt, p_tags))
            n_eol, n_eob = util.degrade_tagged_txt.mixed(mixed_txt_file_path, mixed_tags_file_path, p_eol_add,
            ...
            pp1_ub = (p1 + alpha) / (1 + alpha)
            pp2_ub = (p2 + alpha * p1) / (1 + alpha)
            pp3_ub = (p3 + alpha * p2) / (1 + alpha)
            pp4_ub = (p4 + alpha * p3) / (1 + alpha)
